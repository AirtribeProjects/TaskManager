const express = require('express');
const app = express();
const port = 3000;

app.use(express.json());
app.use(express.urlencoded({ extended: true }));

const aTasks = [
    {
        "id": 1,
        "title": "Set up environment",
        "description": "Install Node.js, npm, and git",
        "completed": true
      },
      {
        "id": 2,
        "title": "Create a new project",
        "description": "Create a new project using the Express application generator",
        "completed": true
      },
      {
        "id": 3,
        "title": "Install nodemon",
        "description": "Install nodemon as a development dependency",
        "completed": true
      },
      {
        "id": 4,
        "title": "Install Express",
        "description": "Install Express",
        "completed": false
      },
      {
        "id": 5,
        "title": "Install Mongoose",
        "description": "Install Mongoose",
        "completed": false
      },
      {
        "id": 6,
        "title": "Install Morgan",
        "description": "Install Morgan",
        "completed": false
      },
      {
        "id": 7,
        "title": "Install body-parser",
        "description": "Install body-parser",
        "completed": false
      },
      {
        "id": 8,
        "title": "Install cors",
        "description": "Install cors",
        "completed": false
      },
      {
        "id": 9,
        "title": "Install passport",
        "description": "Install passport",
        "completed": false
      },
      {
        "id": 10,
        "title": "Install passport-local",
        "description": "Install passport-local",
        "completed": false
      },
      {
        "id": 11,
        "title": "Install passport-local-mongoose",
        "description": "Install passport-local-mongoose",
        "completed": false
      },
      {
        "id": 12,
        "title": "Install express-session",
        "description": "Install express-session",
        "completed": false
      },
      {
        "id": 13,
        "title": "Install connect-mongo",
        "description": "Install connect-mongo",
        "completed": false
      },
      {
        "id": 14,
        "title": "Install dotenv",
        "description": "Install dotenv",
        "completed": false
      },
      {
        "id": 15,
        "title": "Install jsonwebtoken",
        "description": "Install jsonwebtoken",
        "completed": false
      }
]

app.listen(port, (err) => {
    if (err) {
        return console.log('Something bad happened', err);
    }
    console.log(`Server is listening on ${port}`);
});

app.get("/tasks", (req, res) => {
    try {
    if (req.query.completed) {
        const completed = req.query.completed === 'true';
        const filteredTasks = aTasks.filter(task => task.completed === completed);
        return res.send(filteredTasks);
    }
    res.send(aTasks);
    } catch (error) {
        res.status(500).json({ message: 'Error while processing the request' + error });

    }
});

app.get("/tasks/:taskId", (req, res) => {
    try {
        const taskId = req.params.taskId;
        const task = aTasks.find(task => task.id == taskId);
        if (!task) {
            return res.status(404).send("Requested Task not found");
        }
        res.send(task)
    } catch (error) {
        res.status(500).json({ message: 'Error while processing the request' + error });
    }

});

app.post("/tasks", (req, res) => {
    try {
        const task = req.body;
        if (task.id !== undefined) {
            return res.status(400).json({ message: 'ID should not be present in the request body' });
        }
        if (!task.hasOwnProperty("title") || !task.hasOwnProperty("description") ) {
            return res.status(400).json({ message: 'Please provide all the parameter' });
        }
        if (typeof task.completed !== 'boolean' ) {
            return res.status(400).json({ message: 'Invalid Request' });
        }
        task.id = aTasks.length + 1;
        aTasks.push(task);
        res.status(201).send(task);
    } catch (error) {
        res.status(500).json({ message: 'Error while processing the request' + error });
    }
});

app.put("/tasks/:taskId", (req, res) => {
    try {
        const taskId = req.params.taskId;
        const task = aTasks.find(task => task.id == taskId);
        
        if (!task) {
            return res.status(404).send("Task not found for the rquested id");
        }
        task.title = req.body.title;
        task.description = req.body.description;
        task.completed = req.body.completed;
        if (task.completed !== undefined && typeof task.completed !== 'boolean') {
            return res.status(400).json({ message: 'Completed field must be a boolean value' });
        }
        res.send(task);
    } catch (error) {
        res.status(500).json({ message: 'Error while processing the request' + error });
    }
});

app.delete("/tasks/:taskId", (req, res) => {
    try {
        const taskId = req.params.taskId;
        const task = aTasks.find(task => task.id == taskId);
        if (!task) {
            return res.status(404).send("Task not found for the requsted id");
        }
        const index = aTasks.indexOf(task);
        aTasks.splice(index, 1);
        res.status(200).send("Task has been deleted successfull");
    } catch (error) {
        res.status(500).json({ message: 'Error while processing the request' + error });
    }
});

module.exports = app;
